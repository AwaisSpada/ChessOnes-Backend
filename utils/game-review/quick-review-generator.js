...
/**
 * Absolute wall-clock ceiling for one quick-review run.
 * Keeps a wedged engine from blocking the process forever, but must be high enough
 * that authentic LITE analysis on slow hosts (e.g. Render) can finish.
 * Per-move `movetime` still bounds each Stockfish search.
 */
const QUICK_REVIEW_TIMEOUT_MAX_MS = 60 * 60 * 1000; // 1 hour

/**
 * Allotted wall-clock for `generateGameReview`.
 * Earlier move-scaled budgets (~8 min for ~70 plies) aborted authentic LITE runs on
 * shared CPU (Render). Depth/movetime stay unchanged; we allot the full 1h ceiling
 * so analysis can finish. Absolute max still prevents a wedged engine forever.
 * @param {number} _moveCount
 * @param {{ movetime?: number; depth?: number }} [_options]
 * @returns {number}
 */
function computeQuickReviewTimeoutMs(_moveCount, _options = {}) {
  return QUICK_REVIEW_TIMEOUT_MAX_MS;
}
...
async function generateQuickReview(moves, options = {}) {
  ...
  try {
    ...
    console.log(
      `[QuickReview] Full-run timeout cap: ${timeout}ms (${(timeout / 1000).toFixed(0)}s) — authentic LITE kept; cap ${QUICK_REVIEW_TIMEOUT_MAX_MS / 60000}min`
    );
